...
QUnit.test('Testing calculateArea function with several sets of inputs', function (assert) {
  assert.equal(IncomeTax(), 6, 'Tested with two relatively small positive numbers');
  assert.equal(App.IncomeTax(), 9.52, 'Tested two positive non-integer numbers ');
  assert.equal(App.calculateSeats(-5, -5),0, 'Tested with two negative numbers. Any arguments less than 1 will be set to 0.');
  assert.equal(App.calculateSeats(250,250),0, 'Tested with two large numbers. Any argument is set to a huge number.');
  assert.equal(App.calculateSeats('a','b'),0, 'Tested with two String values.Output is 0.');  
  assert.equal(App.calculateSeats('',''),0, 'Tested with two invalid values.Output is 0.');  
});
